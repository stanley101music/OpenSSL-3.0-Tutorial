...
int BF(unsigned char* in, int in_len, unsigned char* key, unsigned char* iv, const EVP_CIPHER* cipher_type, unsigned int crypt_mode, unsigned char*& out)
{
...
int main()
{
    ...
    // Encryption
    ciphertext_len = BF(plaintext, plaintext_len, key, iv, cipher_type, crypt_mode, ciphertext);

    // Decryption
    crypt_mode = 0;
    decryptedtext_len = BF(ciphertext, ciphertext_len, key, iv, cipher_type, crypt_mode, decryptedtext);
