...
int AES(unsigned char* in, int in_len, unsigned char* key, unsigned char* iv, const EVP_CIPHER* cipher_type, unsigned int crypt_mode, unsigned char*& out)
{
    int out_len1, out_len2;
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (crypt_mode == 1)
        out = (unsigned char*)OPENSSL_zalloc(in_len + AES_BLOCK_SIZE);
    else
...
int main()
{
    ...
    // Encryption
    ciphertext_len = AES(plaintext, plaintext_len, key, iv, cipher_type, crypt_mode, ciphertext);

    // Decryption
    crypt_mode = 0;
    decryptedtext_len = AES(ciphertext, ciphertext_len, key, iv, cipher_type, crypt_mode, decryptedtext);
