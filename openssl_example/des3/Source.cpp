...
int DES3(unsigned char* in, int in_len, unsigned char* key, unsigned char* iv, const EVP_CIPHER* cipher_type, unsigned int crypt_mode, unsigned char*& out)
{
    int out_len1, out_len2;
    EVP_CIPHER_CTX* ctx = EVP_CIPHER_CTX_new();
    if (crypt_mode == 1)
        out = (unsigned char*)OPENSSL_zalloc(in_len + DES_BLOCK_SIZE);
    else
...
int main()
{
    ...
    const EVP_CIPHER* cipher_type = EVP_des_ede3_cbc();
    ...
    // Encryption
    ciphertext_len = DES3(plaintext, plaintext_len, key, iv, cipher_type, crypt_mode, ciphertext);

    // Decryption
    crypt_mode = 0;
    decryptedtext_len = DES3(ciphertext, ciphertext_len, key, iv, cipher_type, crypt_mode, decryptedtext);
